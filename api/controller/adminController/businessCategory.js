    ...
    if (businessData) {
      ...
      Business.save((error, saved) => {
        if (error) {
          return res.json({
            status: false,
            message: "Business Category Not Added",
          });
        } else {
          return res.json({
            status: true,
...
var deleteBusinessCategory = (req, res) => {
  try {
    var id = req.body.businessId;
    business.findByIdAndDelete(id).then(
      async (user) => {
        if (user) {
      ...
      (e) => {
        return res.json({
          status: false,
          message: "Business Category Not Deleted",
        });
      }
    );
  } catch (error) {
...
var fetchStatus = (req, res) => {
  try {
    business
      .findById({ _id: mongoose.Types.ObjectId(req.body.businesscategoryId) })
      .then((doc) => {
        if (doc) {
          return res.json({
            status: true,
