...
var CategoryMethod = async (req, res) => {
  try {
    ...
    if (!isCategoryAvailable) {
      ...
      Category.save(async (error, saved) => {
        if (error) {
          return res.json({ status: false, message: "Category Not Added" });
        } else {
          const businesscategory = await business.findOne({
            _id: req.body.businesscategoryId,
          });
          businesscategory.categoryId.push(Category._id);
          businesscategory.save((error, saved) => {
            if (error) {
              return res.json({ status: false, message: "Category Not Added" });
            } else {
              return res.json({
      ...
      return res.json({
        status: false,
        message: "Category Already Available in this business category",
      });
    }
...
var fetchCategory = (req, res) => {
  try {
    category
      .find({ businessId: mongoose.Types.ObjectId(req.body.businesscategory) })
      .populate("businessId", "businesscategory")
      .then((category) => {
        if (category) {
          return res.json({ status: true, message: "", category });
...
var updateCategory = (req, res) => {
  try {
    ...
    category
      .findByIdAndUpdate(id, { $set: { category: req.body.category } })
      .then(
        (user) => {
          if (user) {
            return res.json({
